---
title: "Plantilla de UI de conector web"
description: "Guía para configurar una UI para ingesta web con Firecrawl y React."
og:title: "Plantilla de UI de conector web | Firecrawl"
og:description: "Aprende a configurar una UI para ingesta web con Firecrawl y React."
---
...
Esta plantilla simplifica la creación de una UI de conector web para Firecrawl con React.
Incluye un componente listo que se comunica con la API de Firecrawl, lo que te permite configurar rápidamente una interfaz de rastreo y scraping web para que los usuarios extraigan datos para sus aplicaciones de IA.
![Plantilla de UI de Firecrawl](/images/firecrawl-ui.png)

Esta plantilla permite a los usuarios introducir una URL para rastrearla. Si se detectan subpáginas, la plantilla las rastreará y mostrará una lista de URL descubiertas. Luego, los usuarios podrán seleccionar qué URL scrapear, y el contenido obtenido se ingerirá en su aplicación de IA y se mostrará en la UI.
...
2. Crea un archivo `lib/utils.ts` y agrega el siguiente código:
...
3. Copia y pega nuestro componente principal de ingesta. **Asegúrate de reemplazar `FIRECRAWL_API_KEY` por tu clave.**
...
<Accordion title="Ingestion Component (ingestion.tsx)">
  ...
  //! Se recomienda encarecidamente mover todas las llamadas a la API de Firecrawl al backend (p. ej., una ruta API de Next.js)
  const FIRECRAWL_API_URL = "https://api.firecrawl.dev"; // Reemplaza con tu URL real de la API, ya sea local o usando Firecrawl Cloud
      ...
      try {
            ...
            do {
              ...
              if (statusResponse.ok) {
                ...
                if (statusData.status !== "completed") {
                  ...
                  console.log("El rastreo se completó con estado:", statusData.status);
                  console.log(statusData);
                }
              } else {
                console.error("No se pudo obtener el estado del rastreo");
                break;
              }
        ...
        setScrapeResults({
          error: {
            ...
            data: {
              metadata: {
                pageError: "Se produjo un error al obtener los datos",
                title: "",
                description: "",
                language: "",
                sourceURL: "",
        ...
        <Card>
          <CardHeader className="flex items-start justify-between mb-0 pb-4">
            ...
            <div className="text-sm text-gray-500 w-11/12 items-center">
              Usa este componente para dar rápidamente a tus usuarios la capacidad de conectar
              sus apps de IA con datos web usando Firecrawl. Más información en la{" "}
              <a
                ...
                <CollapsibleContent className="space-y-4 mt-4 px-2">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <label htmlFor="extractMainContent" className="text-sm">
                      Extraer solo el contenido principal (sin encabezados, navegación, pies, etc.)
                    </label>
                  </div>
                </CollapsibleContent>
              </Collapsible>
            ...
            {showCrawlStatus && (
              <div className="flex items-center justify-between mb-2 space-x-2 bg-gray-100 p-2 rounded-md">
                ...
                <div className="text-sm text-gray-600">
                  {isScraping
                    ? `Extraídas ${crawlStatus.current} página(s) en ${elapsedTime}s`
                    : `Rastreadas ${crawlStatus.current} páginas en ${elapsedTime}s`}
                </div>
              </div>
            )}
        ...
        {Object.keys(scrapeResults).length > 0 && (
          <div className="mt-4">
            ...
            <p className="text-sm text-gray-500">
              ...
              una demostración básica del markdown.
            </p>
...
Como no es una biblioteca, puedes personalizar la interfaz a tu gusto. Simplemente edita el código como con shadcn.

<div id="security-considerations">
  ## Consideraciones de seguridad
</div>

Para entornos de producción, considera las siguientes medidas de seguridad:

* Traslada las interacciones con la API a una implementación del lado del servidor para proteger tu clave de la API de Firecrawl.
