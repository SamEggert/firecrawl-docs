...
<div id="using-firecrawl-to-gather-an-entire-website">
  ### Usar Firecrawl para recopilar un sitio web completo
</div>
...
firecrawl_reader = FireCrawlWebReader(
    api_key="<your_api_key>",  # Reemplaza con tu clave de API de https://www.firecrawl.dev/
...
# Establece la variable de entorno para la clave virtual
os.environ["OPENAI_API_KEY"] = "<OPENAI_API_KEY>"

# Carga documentos desde la URL de una sola página
documents = firecrawl_reader.load_data(url="http://paulgraham.com/")
index = SummaryIndex.from_documents(documents)

# Configura el log en DEBUG para obtener resultados más detallados
query_engine = index.as_query_engine()
response = query_engine.query("¿Qué hacía el autor cuando era niño?")
display(Markdown(f"<b>{response}</b>"))
```

<div id="using-firecrawl-to-gather-a-single-page">
  ### Uso de Firecrawl para recopilar una sola página
</div>
