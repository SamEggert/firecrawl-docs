---
title: 'Crawl'
description: 'Firecrawl puede rastrear recursivamente los subdominios de una URL y recopilar su contenido'
og:title: "Crawl | Firecrawl"
og:description: "Firecrawl puede rastrear recursivamente los subdominios de una URL y recopilar su contenido"
icon: "spider"
---
...
1. **Análisis de URL:** Escanea el sitemap y rastrea el sitio web para identificar enlaces
...
4. **Salida:** Convierte los datos en markdown limpio o en un formato estructurado
...
<CodeGroup>
  ...
  // Rastreo con opciones de raspado
...
Si estás usando cURL o el método de inicio, esto devolverá un `ID` para verificar el estado del rastreo.

<Note>Si estás usando el SDK, consulta los métodos a continuación para conocer el comportamiento de espera (waiter) vs inicio (starter).</Note>
...
Se usa para revisar el estado de un trabajo de rastreo y obtener su resultado.

<Note>Este endpoint solo funciona para rastreos que están en curso o que se han completado recientemente.</Note>
...
Para respuestas incompletas o de gran tamaño que superen los 10 MB, se proporciona el parámetro de URL `next`. Debes solicitar esta URL para obtener los siguientes 10 MB de datos. Si el parámetro `next` no está presente, indica el final de los datos del rastreo.

El parámetro skip establece el número máximo de resultados devueltos en cada bloque de resultados.

<Info>
  Los parámetros skip y next solo son relevantes cuando se accede a la API directamente. Si estás usando el SDK, nos encargamos de esto por ti y devolveremos todos los resultados de una vez.
</Info>
...
2. **Iniciar y luego comprobar el estado** (`startCrawl`/`start_crawl`):
...
* `crawl.started` - Cuando comienza el rastreo
* `crawl.page` - Por cada página raspada correctamente
