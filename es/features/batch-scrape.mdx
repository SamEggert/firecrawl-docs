...
description: "Raspar múltiples URL por lotes"
og:title: "Raspado por lotes | Firecrawl"
og:description: "Raspar múltiples URL por lotes"
---
...
Ahora puedes realizar el raspado por lotes de múltiples URL al mismo tiempo. Recibe las URL iniciales y parámetros opcionales como argumentos. El argumento params te permite especificar opciones adicionales para el trabajo de raspado por lotes, como los formatos de salida.

<div id="how-it-works">
  ### Cómo funciona
</div>

Es muy similar al funcionamiento del endpoint `/crawl`. Puedes iniciar el proceso por lotes y esperar a que termine, o iniciarlo y gestionar la finalización por tu cuenta.

* `batchScrape` (JS) / `batch_scrape` (Python): inicia un trabajo por lotes y espera a que se complete, devolviendo los resultados.
* `startBatchScrape` (JS) / `start_batch_scrape` (Python): inicia un trabajo por lotes y devuelve el ID del trabajo para que puedas sondear (polling) o usar webhooks.
...
* Llamar a `startBatchScrape`/`start_batch_scrape` devuelve un ID de trabajo que puedes seguir mediante `getBatchScrapeStatus`/`get_batch_scrape_status`, usando el endpoint de la API `/batch/scrape/{id}` o webhooks. Este endpoint está pensado para comprobaciones en curso o inmediatamente después de la finalización, **ya que los trabajos por lotes expiran a las 24 horas**.
...
También puedes usar el endpoint de raspado por lotes para extraer datos estructurados de las páginas. Esto es útil si quieres obtener los mismos datos estructurados de una lista de URL.
...
Puedes configurar webhooks para recibir notificaciones en tiempo real a medida que se raspee cada URL en tu lote. Esto te permite procesar los resultados de inmediato en lugar de esperar a que se complete todo el lote.
...
* `batch_scrape.started` - Cuando se inicia el raspado por lotes
* `batch_scrape.page` - Para cada URL raspada correctamente
* `batch_scrape.completed` - Cuando se procesan todas las URL
* `batch_scrape.failed` - Si el raspado por lotes presenta un error
