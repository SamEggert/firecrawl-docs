...
[Firecrawl](https://firecrawl.dev?ref=github) es un servicio de API que toma una URL, la rastrea y la convierte en Markdown limpio. Rastreamos todas las subpáginas accesibles y te entregamos Markdown limpio para cada una. No se requiere un sitemap.
...
**Self-host:** Para alojarlo tú mismo, consulta la guía [aquí](/es/contributing/self-host).
...
Se usa para rastrear una URL y todas las subpáginas accesibles. Esto envía un job de rastreo y devuelve un ID de job para comprobar el estado del rastreo.
...
<CodeGroup>
  ...
  // Registra el markdown en la consola
...
Si no estás usando el SDK o prefieres usar un webhook o un método de sondeo diferente, puedes establecer `wait_until_done` en `false`.
Esto devolverá un jobId.

Para cURL, /crawl siempre devolverá un jobId que puedes usar para verificar el estado del rastreo.
...
<div id="check-crawl-job">
  ### Comprobar el estado de un rastreo
</div>
...
Para extraer una única URL, utiliza el método `scrape_url`. Recibe la URL como parámetro y devuelve los datos extraídos como un diccionario.
...
```json
{
  ...
  "data": {
    ...
    "metadata": {
      ...
      "language": "<string>",
      "sourceURL": "<string>",
      "<cualquier otro metadato> ": "<string>",
      "pageStatusCode": 123,
      "pageError": "<string>"
    },
    "llm_extraction": {},
...
Con la extracción mediante LLM, puedes extraer fácilmente datos estructurados de cualquier URL. Compatibilizamos esquemas de Pydantic para facilitarte el proceso. Así es como se usa:
...
<CodeGroup>
  ...
  top: List[ArticleSchema] = Field(..., max_items=5, description="Las 5 principales historias")
  ...
  params := map[string]any{
    "extractorOptions": ExtractorOptions{
      Mode:             "llm-extraction",
      ExtractionPrompt: "Según la información de la página, identifica cuál es la misión de la empresa, si admite SSO y si es de código abierto",
