---
title: 'Rust'
description: 'El SDK de Rust de Firecrawl es una biblioteca que te ayuda a extraer y rastrear sitios web fácilmente y a generar los datos en un formato listo para usar con modelos de lenguaje (LLMs).'
icon: 'rust'
og:title: "Rust SDK | Firecrawl"
og:description: "El SDK de Rust de Firecrawl es una biblioteca que te ayuda a extraer y rastrear sitios web fácilmente y a generar los datos en un formato listo para usar con modelos de lenguaje (LLMs)."
---

> Nota: esto utiliza la [versión v0 de la API de Firecrawl](/es/v0/introduction), que está en proceso de desuso. Recomendamos cambiar a [v1](/es/sdks/rust).
...
async fn main() {
  ...
  match scrape_result {
    ...
    Err(e) => eprintln!("Se produjo un error durante el raspado: {}", e),
  }
  ...
  match crawl_result {
    ...
    Err(e) => eprintln!("Se produjo un error durante el rastreo: {}", e),
  }
}
```
...
match scrape_result {
  Ok(data) => println!("Datos extraídos: {}", data),
  Err(e) => eprintln!("Error al extraer la URL: {}", e),
}
```
...
Para verificar el estado de un job de rastreo, utiliza el método `check_crawl_status`. Recibe el ID del job como parámetro y devuelve el estado actual del rastreo.
...
Para cancelar un trabajo de rastreo, utiliza el método `cancel_crawl_job`. Recibe el ID del trabajo como parámetro y devuelve el estado de la cancelación del rastreo.

```rust
let job_id = "tu_job_id_aquí";
...
Con la extracción mediante LLM, puedes extraer fácilmente datos estructurados de cualquier URL. Así es como usarla:
...
let json_schema = json!({
  ...
  "properties": {
    "top": {
      ...
      "minItems": 5,
      "maxItems": 5,
      "description": "Las 5 principales historias en Hacker News"
    }
  },
  "required": ["top"]
});
...
Para buscar en la web, obtener los resultados más relevantes, extraer cada página y devolver el contenido en Markdown, usa el método `search`. El método recibe la consulta como parámetro y devuelve los resultados de la búsqueda.
---MDX&#95;CONTENTEND---
