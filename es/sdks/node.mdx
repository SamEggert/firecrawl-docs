---
title: 'Node'
description: 'El SDK de Firecrawl para Node es un wrapper de la API de Firecrawl que te ayuda a convertir fácilmente sitios web a Markdown.'
icon: 'node'
og:title: "Node SDK | Firecrawl"
og:description: "El SDK de Firecrawl para Node es un wrapper de la API de Firecrawl que te ayuda a convertir fácilmente sitios web a Markdown."
---
...
Para extraer una sola URL con manejo de errores, usa el método `scrapeUrl`. Recibe la URL como parámetro y devuelve los datos extraídos como un diccionario.
...
Para rastrear un sitio web con manejo de errores, usa el método `crawlUrl`. Recibe la URL inicial y parámetros opcionales como argumentos. El argumento `params` te permite especificar opciones adicionales para el trabajo de rastreo, como el número máximo de páginas a rastrear, los dominios permitidos y el formato de salida. Consulta [Paginación](#pagination) para la paginación automática o manual y los límites.
...
Inicia un trabajo sin esperar usando `startCrawl`. Devuelve un `ID` de trabajo que puedes usar para comprobar el estado. Usa `crawl` cuando quieras un “esperador” que bloquee hasta que termine. Consulta [Paginación](#pagination) para el comportamiento y los límites de paginación.
...
Para cancelar un trabajo de rastreo, usa el método `cancelCrawl`. Recibe el ID del trabajo de `startCrawl` como parámetro y devuelve el estado de la cancelación.

<CancelCrawlNodeShort />

<div id="mapping-a-website">
  ### Crear un mapa de un sitio web
</div>

Para mapear un sitio web con manejo de errores, utiliza el método `mapUrl`. Recibe la URL inicial como parámetro y devuelve los datos mapeados como un diccionario.
...
Los endpoints de Firecrawl para crawl y batch devuelven una URL `next` cuando hay más datos disponibles. El SDK de Node pagina automáticamente por defecto y agrega todos los documentos; en ese caso, `next` será `null`. Puedes desactivar la paginación automática o establecer límites.

<div id="crawl">
  #### Rastreo
</div>

Usa el método de espera `crawl` para la experiencia más sencilla, o inicia un trabajo y pagina manualmente.

<div id="simple-crawl-auto-pagination-default">
  ##### Rastreo simple (paginación automática, predeterminado)
</div>

* Consulta el flujo predeterminado en [Rastreo de un sitio web](#crawling-a-website).
...
* Mantén la paginación automática activada, pero detén antes con `maxPages`, `maxResults` o `maxWaitTime`.
...
Usa el método del waiter `batchScrape`, o inicia un job y pagina manualmente.
...
El SDK gestiona los errores devueltos por la API de Firecrawl y lanza las excepciones correspondientes. Si se produce un error durante una solicitud, se lanzará una excepción con un mensaje descriptivo. Los ejemplos anteriores muestran cómo manejar estos errores usando bloques `try/catch`.