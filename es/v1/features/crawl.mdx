---
title: 'Rastreo'
description: 'Firecrawl puede explorar de forma recursiva los subdominios de una URL y recopilar su contenido'
og:title: "Rastreo | Firecrawl"
og:description: "Firecrawl puede explorar de forma recursiva los subdominios de una URL y recopilar su contenido"
---
...
1. **Análisis de URL:** Escanea el sitemap y recorre el sitio web para identificar enlaces
2. **Recorrido:** Sigue los enlaces de forma recursiva para encontrar todas las subpáginas
3. **Raspado:** Extrae el contenido de cada página, gestionando JS y límites de tasa
...
Si estás usando cURL o funciones de `async crawl` en los SDK, se devolverá un `ID` que puedes usar para consultar el estado del rastreo.

<Note>Si usas el SDK, consulta la sección de respuesta del SDK [más abajo](#sdk-response).</Note>

<AsyncCrawlOutput />

<div id="check-crawl-job">
  ### Consultar trabajo de crawl
</div>

Se utiliza para comprobar el estado de un trabajo de crawl y obtener su resultado.

<Note>Este endpoint solo funciona para crawls que están en curso o que se han completado recientemente.</Note>
...
Para respuestas no finalizadas o que superen los 10 MB, se proporciona un parámetro de URL `next`. Debes solicitar esa URL para obtener los siguientes 10 MB de datos. Si el parámetro `next` no está presente, indica el final de los datos del rastreo.

El parámetro skip establece el número máximo de resultados que se devuelven en cada segmento.

<Info>
  Los parámetros skip y next solo son relevantes cuando llamas a la API directamente. Si usas el SDK, nosotros nos encargamos de esto y te devolveremos todos los resultados de una vez.
</Info>
...
El SDK ofrece dos formas de rastrear URLs:

1. **Rastreo síncrono** (`crawl_url`/`crawlUrl`):
   * Espera a que el rastreo finalice y devuelve la respuesta completa
...
2. **Rastreo asíncrono** (`async_crawl_url`/`asyncCrawlUrl`):
   * Devuelve inmediatamente un ID de rastreo
   * Permite comprobar el estado manualmente
...
Acelera tus rastreos hasta un 500% cuando no necesitas los datos más recientes. Añade `maxAge` a tus `scrapeOptions` para usar la página en caché cuando esté disponible.
...
* Cada página del rastreo comprueba si hay datos en caché más nuevos que `maxAge`
* Si los hay, se devuelve al instante desde la caché (hasta 500% más rápido)
* Si no, se raspa la página de nuevo y se almacena el resultado en la caché
* Ideal para rastrear sitios de documentación, catálogos de productos u otro contenido relativamente estático

Para más detalles sobre el uso de `maxAge`, consulta la documentación de [Faster Scraping](/es/features/fast-scraping).

<div id="crawl-websocket">
  ## WebSocket de rastreo
</div>

El método de Firecrawl basado en WebSocket, `Crawl URL and Watch`, permite extraer y monitorear datos en tiempo real. Inicia un rastreo con una URL y personalízalo con opciones como límites de páginas, dominios permitidos y formatos de salida; es ideal para necesidades de procesamiento de datos inmediato.
...
Puedes configurar webhooks para recibir notificaciones en tiempo real a medida que avanza el rastreo. Esto te permite procesar las páginas conforme se van raspando, en lugar de esperar a que finalice todo el rastreo.
...
* `crawl.started` - Cuando inicia el rastreo
...
```json
{
  ...
  "type": "crawl.page",
  "id": "crawl-job-id",
  "data": [...], // Datos de la página para los eventos 'page'
...
<Note>
  Para ver la configuración detallada de los webhooks, las mejores prácticas de seguridad y la solución de problemas, visita la [documentación de Webhooks](/es/features/webhooks).
</Note>
